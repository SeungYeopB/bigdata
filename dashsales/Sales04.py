...
def update_graph(select_years, radio_items):
    ...
    return{
        ...
        'layout': go.Layout(
            title={'text': '판매 트렌드 in year' + ' ' + str((select_years)),
                   'y': 0.99,
                   'x': 0.5,
                   'xanchor': 'center',
                   'yanchor': 'top'},
...
def update_graph(select_years, radio_items, radio_items2):
    ...
    sales13 = sales12[(sales12['Year'] == select_years) & (sales12['Segment'] == \
               radio_items)].sort_values(by = ['Sales'], ascending = False).nlargest(10, columns = ['Sales'])
    sales14 = sales.groupby(['Year', 'City', 'Segment'])['Sales'].sum().reset_index()
    sales15 = sales14[(sales14['Year'] == select_years) & (sales14['Segment'] == \
               radio_items)].sort_values(by=['Sales'],ascending=False).nlargest(10, columns = ["Sales"])
     ...
     return {
        'data': [
            go.Bar(
                ...
                hoverinfo='text',
                hovertext=
                '<b>년도</b>: ' + sales13['Year'].astype(str) + '<br>' +
                '<b>고객구분</b>: ' + sales13['Segment'].astype(str) + '<br>' +
                '<b>State</b>: ' + sales13['State'].astype(str) + '<br>' +
                '<b>판매금액</b>: $' + [f'{x:,.0f}' for x in sales13['Sales']] + '<br>'
            ),
        ],
    ...
    return{
        'data': [
            go.Scatter(
                ...
                hoverinfo='text',
                hovertext=
                '<b>년도</b>: ' + sales18['Year'].astype(str) + '<br>' +
                '<b>월</b>: ' + sales18['Month'].astype(str) + '<br>' +
                '<b>고객구분</b>: ' + sales18['Segment'].astype(str) + '<br>' +
                '<b>State</b>: ' + sales18["State"].astype(str) + '<br>' +
                '<b>도시</b>: ' + sales18["City"].astype(str) + '<br>' +
                '<b>판매금액</b>: $' + [f"{x:,.0f}" for x in sales18["Sales"]] + "<br>"

            ),

        ],
        'layout': go.Layout(
            title={'text': 'State and 도시별 판매 in year' + ' ' + str((select_years)),
                   'y': 0.99,
                   'x': 0.5,
                   'xanchor': 'center',
                   'yanchor': 'top'},
