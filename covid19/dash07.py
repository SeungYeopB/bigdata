...
def update_graph(w_countries):
    covid_data_2 = covid_data.groupby(["date", "Country/Region"]) \
        [["confirmed", "death", "recovered", "active"]].sum().reset_index()
    confirmed_value = covid_data_2[covid_data_2["Country/Region"] \
        == w_countries]["confirmed"].iloc[-1]
    death_value = covid_data_2[covid_data_2["Country/Region"] \
        == w_countries]["death"].iloc[-1]
    recovered_value = covid_data_2[covid_data_2["Country/Region"] \
        == w_countries]["recovered"].iloc[-1]
    active_value = covid_data_2[covid_data_2["Country/Region"] \
        == w_countries]["active"].iloc[-1]
    colors = ["orange", "#ddle35", "green", "#e55467"]
    ...
    return {
        ...
        "layout": go.Layout(
            title={"text": "Total Class:" + (w_countries),
                   "y": 0.93,
                   "x": 0.5,
                   "xanchor": "center",
                   "yanchor": "top"},
...
def update_graph(w_countries):
    covid_data_2 = covid_data.groupby(["date", "Country/Region"]) \
        [["confirmed", "death", "recovered", "active"]].sum().reset_index()
    covid_data_3 = covid_data_2[covid_data_2["Country/Region"] \
        == w_countries][["Country/Region", "date","confirmed"]].reset_index()
    covid_data_3["daily confirmed"] = covid_data_3["confirmed"]\
        - covid_data_3["confirmed"].shift(1)
    ...
    return {
        "data": [go.Bar(
            ...
            "<br>Country</br>: " + covid_data_3["Country/Region"].tail(30).astype(str) + "<br>"
        ),
